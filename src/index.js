module.exports = function toReadable(number) {
    ...
    if (num == 0) {
       ...
       return `${uno[+num[0]]}`;
        ...
        return (+num[0] === 1 ? `${tenTh[+num[1]]}` : `${dec[+num[0]]} ${uno[+num[1]]}`).split('  ').join(' ').trim();
    }

    else if (num.length === 3) {
        return (+num[1] === 1 ? `${uno[+num[0]]} hundred ${tenTh[+num[2]]}` : `${uno[+num[0]]} hundred ${dec[+num[1]]} ${uno[+num[2]]}`).split('  ').join(' ').trim();
    }
